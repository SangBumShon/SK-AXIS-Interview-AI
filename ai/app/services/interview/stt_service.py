...
def transcribe_audio_file(file_path: str) -> str:
    ...
    print(f"[STT] 🎯 Whisper 원본 결과: '{result}'")
...
def test_stt_filtering():
    """
    STT 필터링 기능을 테스트하는 함수
    """
    test_cases = [
        "오늘도 시청해 주셔서 감사합니다",
        "시청해주셔서 감사합니다",
    ...
    for test_text in test_cases:
        is_filtered = is_invalid_transcription(test_text)
        status = "❌ 필터링됨" if is_filtered else "✅ 통과"
        print(f"  '{test_text}' → {status}")
    print("[STT 필터링 테스트 완료]\n")
