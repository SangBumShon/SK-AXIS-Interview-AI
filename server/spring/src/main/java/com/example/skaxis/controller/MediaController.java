...
import com.example.skaxis.dto.InterviewScheduleParseResponseDto;
...
//
//        FileUploadResponseDto response = mediaService.uploadSttFile(file);
//        return ResponseEntity.ok(response);
//    }
...
// MediaController 클래스에 추가할 메서드

    @PostMapping(value = "/upload/interview-schedule", consumes = MediaType.MULTIPART_FORM_DATA_VALUE)
    @Operation(summary = "면접 일정 엑셀 파일 업로드", description = "면접 일정이 포함된 엑셀 파일을 업로드합니다.")
    public ResponseEntity<FileUploadResponseDto> uploadInterviewScheduleExcel(
            @Parameter(description = "업로드할 면접 일정 엑셀 파일 (.xlsx, .xls)")
            @RequestParam("file") MultipartFile file) throws IOException {

//        FileUploadResponseDto response = mediaService.uploadInterviewScheduleExcel(file);
        FileUploadResponseDto response = mediaService.uploadAndProcessInterviewScheduleExcel(file);
        return ResponseEntity.ok(response);
    }

//    @PostMapping("/parse/interview-schedule")
//    @Operation(summary = "면접 일정 엑셀 파일 파싱 및 저장", description = "업로드된 면접 일정 엑셀 파일을 파싱하여 데이터베이스에 저장합니다.")
//    public ResponseEntity<InterviewScheduleParseResponseDto> parseInterviewScheduleExcel(
//            @RequestBody ExcelParseRequestDto request) {
//
//        InterviewScheduleParseResponseDto response = mediaService.uploadAndProcessInterviewScheduleExcel(file)
//        return ResponseEntity.ok(response);
//    }
}